...
function tailwindBuild() {
	...
	return src(v.src.styleTailwind)
		.pipe(plumber({ errorHandler: notify.onError("Error: <%= error.message %>") }))
...
function watchServBuild() {
	...
	watch(v.watch.styleTailwind, tailwindBuild)
...
const filesBuild = parallel(htmlBuild, jsBuild, jsModulesBuild, cssBuild, tailwindBuild, fontsBuild, otherBuild);
...
function watchServerProd() {
	...
	watch(v.watch.styleTailwind, tailwindProd)
...
const filesProd = parallel(htmlProd, jsProd, jsModulesProd, cssProd, tailwindProd, fontsProd, otherProd);
